alter table public.reminder_preferences
  add column if not exists compliance_recipient_emails text[] not null default '{}'::text[];
...
begin
  ...
  cross join lateral unnest(coalesce(rp.reminder_days, array[30,7,2])) as rd(day)
