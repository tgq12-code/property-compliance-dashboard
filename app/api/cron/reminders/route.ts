...
export async function GET(request: Request) {
  ...
  for (const item of obligations) {
    try {
      ...
      const plainText = ["VO FAMILY COMPLIANCE REMINDER", "", status, item.title, context, "", `Due date: ${due}`, amount ? `Amount: ${amount}` : null, item.official_payment_url ? `Official link: ${item.official_payment_url}` : null, "", item.reminder_day < 0 ? "This item appears past due. Please verify whether it has already been completed or paid." : "Please review this item before the due date.", "", "Sent automatically by the Vo Family Reminder system."].filter(Boolean).join("\n");
      ...
      const html = `<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;color:#111827;line-height:1.6"><div style="font-size:12px;letter-spacing:.14em;color:#2563eb;font-weight:700">VO FAMILY COMPLIANCE REMINDER</div><h1 style="font-size:24px;margin:10px 0 6px">${escapeHtml(item.title)}</h1><p style="margin:0 0 20px;color:#6b7280">${escapeHtml(context)}</p><div style="padding:18px;border-radius:14px;background:${item.reminder_day < 0 ? "#fef2f2" : item.reminder_day === 0 ? "#fff7ed" : "#eff6ff"}"><div style="font-weight:700;font-size:18px">${escapeHtml(status)}</div><div style="margin-top:8px"><strong>Due:</strong> ${escapeHtml(due)}</div>${amount ? `<div><strong>Amount:</strong> ${escapeHtml(amount)}</div>` : ""}</div>${actionButton}<p style="margin-top:24px;font-size:12px;color:#9ca3af">Automatic reminders are scheduled at 60, 30, 14, 7, and 2 days before the due date, on the due date, and after the due date if still open. Escrowed property-tax items are excluded.</p></div>`;
